...
Please check the Github page for more information about this release."""
            ...
            sizer.AddSpacer(10)
            self.title_text = wx.StaticText(panel, label="A new version of OCLP-Mod is available!")
            self.description = wx.StaticText(panel, label=f"OCLP-Mod {version} is now available - You have {self.constants.patcher_version}{' (Nightly)' if not self.constants.commit_info[0].startswith('refs/tags') else ''}. Would you like to update?")
...
'''
        ...
        if utilities.check_seal() is True:
            ...
            if patches:
                ...
                #    warning_str = f"""\n\nWARNING: We're unable to verify whether there are any new releases of OCLP-Mod on Github. Be aware that you may be using an outdated version for this OS. If you're unsure, verify on Github that OCLP-Mod {self.constants.patcher_version} is the latest official release"""
    ...
    def _determine_if_versions_match(self):
        ...
        args = [
            "/usr/bin/osascript",
            "-e",
            f"""display dialog "OCLP-Mod has detected that you are booting {'a different' if self.constants.special_build else 'an outdated'} OpenCore build\n- Booted: {self.constants.computer.oclp_version}\n- Installed: {self.constants.patcher_version}\n\nWould you like to update the OpenCore bootloader?" """
            f'with icon POSIX file "{self.constants.app_icon_path}"',
        ]
        output = subprocess.run(
            args,
