...
class GenerateDefaults:
    ...
    def _detect_china_mainland(self) -> bool:
        ...
        try:
            ...
            # 获取当前时区偏移（小时）
            # time.timezone：本地时间与UTC的秒数偏移（西时区为正，东时区为负）
            offset = time.timezone / -3600  # 转换为小时
            
            # 判断是否为UTC+8（允许0.5小时误差）
            is_china = abs(offset - 8) < 0.5
            
            # 设置常量值
            self.constants.use_simplehacapi = is_china
            
            logging.info(f"时区检测完成: UTC偏移={offset}, 中国大陆={is_china}")
            return is_china
            
        except Exception as e:
            logging.error(f"时区检测失败: {e}")
            # 检测失败时默认设为False
            self.constants.use_simplehacapi = False
            return False
