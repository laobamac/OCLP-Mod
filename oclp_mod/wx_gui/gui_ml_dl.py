...
class LoadingFrame(wx.Frame):
    def __init__(self, parent,global_constants: constants.Constants, title):
        ...
        # Title
        self.title_label = wx.StaticText(panel, label=self.language_handler.get_translation("Retrieving_MetalLib_information..."), pos=(50, 10))
        self.title_label.SetFont(wx.Font(12, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD))
...
class DownloadMLFrame(wx.Frame):
    def __init__(self, parent,global_constants: constants.Constants,):
        self.constants: constants.Constants = global_constants
        self.language_handler = LanguageHandler(self.constants)
        super(DownloadMLFrame, self).__init__(parent, title=self.language_handler.get_translation("MetalLib_download"), size=(600, 400))
        panel = wx.Panel(self)

        self.list_ctrl = DownloadListCtrl(panel,self.constants)
        self.download_button = wx.Button(panel, label=self.language_handler.get_translation("Download"), pos=(250, 350))
    ...
    def fetch_MetalLib_data(self):
        ...
        try:
            ...
            wx.MessageBox(f"{self.language_handler.get_translation('Failed_to_retrieve_MetalLib_information:')} {e}", self.language_handler.get_translation('Error'), wx.OK | wx.ICON_ERROR)
            wx.CallAfter(self.loading_frame.close)
    ...
    def on_copy(self, event):
        ...
        if selected_data:
            ...
            wx.MessageBox(self.language_handler.get_translation("Please_select_a_version_of_MetaLib_to_copy."), self.language_handler.get_translation("Guide"), wx.OK | wx.ICON_INFORMATION)

    def on_download(self, event):
        selected_data = self.list_ctrl.get_selected_data()
        if selected_data:
            with wx.FileDialog(self, self.language_handler.get_translation("Save_the_file"), wildcard="PKG Files (*.pkg)|*.pkg", style=wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT) as dlg:
            ...
            wx.MessageBox(self.language_handler.get_translation("Please_select_a_MetalLib_version_to_download."), self.language_handler.get_translation("Guide"), wx.OK | wx.ICON_INFORMATION)